<div wire:key="{{$todo->id}}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
                <div class="flex justify-between space-x-2">
                  <div class="flex items-center">
                    @if ($todo->completed)
                    <input wire:click="toggleComplete({{$todo->id}})" class="mr-2" type="checkbox" checked>
                    <h3 class="text-lg text-semibold line-through text-gray-800">{{$todo->name}}</h3>
                    @else
                    <input wire:click="toggleComplete({{$todo->id}})" class="mr-2" type="checkbox" >
